...
- **Targets multiple platforms** — Infineon PSoC 6 BLE today (Phase I),
  Raspberry Pi 5 (POSIX / Linux) and Nordic nRF5340 (Zephyr / nRF Connect SDK)
  in Phase III, all behind the same platform-agnostic abstractions.

The intended companion is a **`sentinel-client-ios`** Swift application
(separate repo, planned). The firmware is designed to pair with it without
client-side assumptions about which platform served the data.
...
|---|---|---|
| **I — MVP on Infineon CYBLE-416045** | Environmental sensing, event log, snapshot history, live snapshot stream, OTA DFU | **In progress** — 10 / 19 issues closed |
| **II — Driver expansion + refinements** | Full driver backlog (14 chips spanning I²C / SPI / GPIO / PWM / 1-Wire), BME280 SPI mode, refinements to Phase I | Not started |
| **III — Multi-platform support** | Port to Raspberry Pi 5 (POSIX / Linux) and Nordic nRF5340 (Zephyr) | Not started |
...
│                                                                │
│   CYHAL today (Phase I) — Zephyr / NCS in Phase III for nRF5340 │
...
- **HAL:** Infineon CYHAL (Phase I); Zephyr drivers (Phase III, nRF5340);
  POSIX + libgpiod (Phase III, Pi 5).
- **BLE stack:** AIROC (Cypress / Infineon) Bluetooth LE stack (Phase I);
  Zephyr Bluetooth subsystem (Phase III, nRF5340); BlueZ (Phase III, Pi 5).
