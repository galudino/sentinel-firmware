...
///          the shared system bring-up (\ref
///          sentinel::resource::system_initialize), create this target's
///          one-shot orchestrator via the common
...
///          orchestrator #48); the Makefile \c CY_IGNORE's the other dir, so
///          the linker resolves exactly one definition with \b no \c \#ifdef
///          here. See \ref sentinel_orchestrator_entry.hpp.
///
/// \author  galudino
/// \date    2026-05-15
...
int main(int argc, const char *argv[]) {
    ...
    // arbiters can pump the I/O its work issues (decision #13). The GATT DB
    // (#6) registers asynchronously in BTM_ENABLED; the orchestrator reads that
    // real result live at POST, so the argument here is only a pre-scheduler
    // fallback.
