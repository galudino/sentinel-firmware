...
public:
    ...
    ///          "major.minor.tweak.small" (e.g., "2.1.3.15").
    ///
    ...
    /// \param build Build version number (0-255). Default is APP_VERSION_BUILD.
    ///
    ...
    constexpr explicit firmware_version(uint8_t major = APP_VERSION_MAJOR,
                                        ...
                                        uint8_t build = APP_VERSION_BUILD)
        : m_major(major), m_minor(minor), m_patch(patch), m_build(build),
          m_buf{} {
        auto pos = 0;
        pos += append_uint(m_major, m_buf + pos);
        m_buf[pos++] = '.';
        pos += append_uint(m_minor, m_buf + pos);
        m_buf[pos++] = '.';
        pos += append_uint(m_patch, m_buf + pos);
        m_buf[pos] = '\0';
        pos += append_uint(m_build, m_buf + pos);
        m_buf[pos++] = '.';
    }
    ...
    ///
    /// \brief Get version components as std::array
    ///
    /// \details Returns the four version components as a std::array for
    ///          convenient access and iteration. The array contains the
    ///          version components in order: [major, minor, tweak, small].
    ///
    /// \return std::array<uint8_t, 4> containing version components
    ///
    ...
    ///
    constexpr std::array<uint8_t, 4> array() const {
        return std::array<uint8_t, 4>{m_major, m_minor, m_patch, m_build};
    }
...
private:
    ...
    ///
    static constexpr int append_uint(uint8_t value, char *out) {
        auto len = 0;
        if (value >= 100) {
    ...
    uint8_t m_build; ///< Build version component (0-255)

    /// Internal buffer for version string storage
    /// Size: "255.255.255.255\0" = 16 characters maximum
    char m_buf[16];
};
...
///
constexpr auto current_firmware_version = firmware_version{0, 0, 0, 0};
