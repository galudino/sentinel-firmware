...
| **Issues** | #6 (GATT catalog), #45 (DIS + Platform ID), #55 (CPU temperature char), #69 (device readiness), #72 (paged blob-read fix) |
...
| `gatt::system` | [`sentinel_gatt_system.hpp`](sentinel_gatt_system.hpp) | System service: identity, CPU temperature, device readiness (boot/POST state, R/Notify), bootloader |
