 ...
 */
#define CY_OTA_NEXT_CHECK_INTERVAL_SECS                                        \
    (24 * 60 * 60) /* 1 day between checks. */

/**
 * @brief Retry time after which to check for OTA updates again.
 *
 * This is used to restart the timer after failing to contact the server during
 * an OTA update check.
 */
#define CY_OTA_RETRY_INTERVAL_SECS                                             \
    (5) /* 5 seconds between retries after an error. */
 ...
 *
 * This is used to determine # retries when connecting to the server during an
 * OTA update check.
 */
 ...
 * runtime. Override if desired by defining in cy_ota_config.h.
 */
